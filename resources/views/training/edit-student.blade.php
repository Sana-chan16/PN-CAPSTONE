    ...
    <form action="{{ route('training.students.update', $student->user_id) }}" method="POST" id="studentForm" onsubmit="return validateForm()">
        ...
        <div class="form-group">
            ...
            <input type="number" name="grade" id="grade" class="form-control" 
                   ...
                   onchange="validateGrade(this.value)">
            <small class="text-muted">Passing: 1.0-3.0 | Failing: 3.1-5.0</small>
        </div>
...
function validateGrade(grade) {
    ...
    if (isNaN(gradeValue) || gradeValue < 1.0 || gradeValue > 5.0) {
        gradeInput.setCustomValidity('Grade must be between 1.0 and 5.0');
    } else {
    ...
    if (status) {
        if (gradeValue >= 1.0 && gradeValue <= 3.0) {
            status.value = 'Passed';
        } else if (gradeValue >= 3.1 && gradeValue <= 5.0) {
            status.value = 'Failed';
        }
    }
}
